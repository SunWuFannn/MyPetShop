...
public class AdminEditController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            String accessID = request.getParameter("txtID");
            ProcessAccessory bean = new ProcessAccessory();
            bean.setAccessoryID(accessID);
            AccessoryDTO dto = bean.findByPK();
            if(dto != null){
                url = "/Admin/editAccessory.jsp";
                request.setAttribute("INFO", dto);
            }
            else{
                request.setAttribute("ERROR", "Failed at editcontroller");
            }
        } catch (Exception e) {
            log("Error at admin edit controller: " + e.getMessage());
        }
        finally{
