 ...
 * @author 99hai
 */
public class AdminOpenService extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            valid = bean.openService();
            if (valid) {
                url = SUCCESS;
                HttpSession session = request.getSession();
                session.setAttribute("CHECK", 1);
            } else {
                request.setAttribute("ERROR", "Falied when open service");
            }
        } catch (Exception e) {
            log("Error at AdminOpenService : " + e.getMessage());
        } finally {
            if (valid) {
